...
class TouchImageView : ImageView {
    ...
    val zoomedRect: RectF
        ...
        get() {
            ...
            val w = getDrawable().intrinsicWidth.toFloat()
            val h = getDrawable().intrinsicHeight.toFloat()
    ...
    private fun fitImageToView() {
        ...
        when (mScaleType) {
            ...
            ScaleType.CENTER_INSIDE -> {
                run {
                    scaleY = min(1.0, min(scaleX.toDouble(), scaleY.toDouble()))
                        .toFloat()
                    scaleX = scaleY
                }
                run {
    ...
    private fun setViewSize(mode: Int, size: Int, drawableWidth: Int): Int {
        ...
        viewSize = when (mode) {
            ...
            MeasureSpec.AT_MOST -> min(drawableWidth.toDouble(), size.toDouble())
                .toInt()
    ...
    private fun transformCoordTouchToBitmap(x: Float, y: Float, clipToBitmap: Boolean): PointF {
        matrix!!.getValues(m)
        val origW = getDrawable().intrinsicWidth.toFloat()
        val origH = getDrawable().intrinsicHeight.toFloat()
        val transX = m!![Matrix.MTRANS_X]
        val transY = m!![Matrix.MTRANS_Y]
