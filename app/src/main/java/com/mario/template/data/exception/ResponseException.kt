...
import com.mario.template.data.remote.response.ErrorResponse
...
class ResponseException(
    ...
    private var _errorServer: ErrorResponse? = null
     ...
     */
    fun getErrorData(): ErrorResponse? = _errorServer

    fun deserializeServerError() {
        val responseBody = response?.errorBody()
        if (responseBody != null) {
            try {
                _errorServer = responseBody.getErrorBodyAs(ErrorResponse::class.java)
                responseBody.close()
            } catch (e: IOException) {
